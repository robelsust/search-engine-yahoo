...
use Serps\SearchEngine\Yahoo\Parser\Evaluated\AdwordsParser as EvaluatedAdwordsParser;
use Serps\SearchEngine\Yahoo\Parser\Evaluated\MobileNaturalParser;
use Serps\SearchEngine\Yahoo\Parser\Evaluated\NaturalParser as EvaluatedNaturalParser;
use Serps\Stubs\RelatedSearch;

class YahooSerp extends YahooDom
{
    ...
    public function javascriptIsEvaluated()
    {
        ...
        /** @var $body \DOMElement */
      $class = $body->getAttribute('id');
		
        if ($class=='hsrp') {//for captcha
            return false;
        } elseif ($class=='ysch') {
            return true;
        } else {
